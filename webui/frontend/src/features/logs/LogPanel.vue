<template>
  <div class="log-console">
    <div class="log-tools">
      ...
      <select v-model="tail" aria-label="日志行数"><option value="200">200 行</option><option value="1000">1000 行</option><option value="5000">5000 行</option></select>
      <button class="mini secondary" type="button" @click="wrap = !wrap">{{ wrap ? "关闭换行" : "开启换行" }}</button>
      <button class="mini secondary" type="button" @click="errorsOnly = !errorsOnly">{{ errorsOnly ? "全部日志" : "只显示错误" }}</button>
...
import { fetchTaskLogs } from "../../stores/taskStore.js";
...
const displayed = computed(() => {
  const lines = errorsOnly.value ? rawLines.value.filter((line) => /error|fail|exception|panic|ERR/i.test(line)) : rawLines.value;
  return lines.join("\n");
});
async function fetchLogs() {
  ...
  error.value = "";
  try {
    const text = await fetchTaskLogs(props.unit, Number(tail.value));
    if (currentRequest !== requestId) return;
    rawLines.value = text.split("\n");
    await nextTick();
    if (logEl.value) logEl.value.scrollTop = logEl.value.scrollHeight;
  } catch (reason) {
    if (currentRequest === requestId) error.value = reason.message || "日志加载失败";
  } finally {
    if (currentRequest === requestId) loading.value = false;
  }
}
function clear() { rawLines.value = []; toast("日志已清屏，自动刷新后会重新显示"); }
async function copy() {
  try {
...
onMounted(() => {
  fetchLogs();
  timer = setInterval(() => { if (autoLog.value) fetchLogs(); }, 5000);
});

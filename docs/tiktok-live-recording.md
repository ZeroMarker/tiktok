...
- 昵称抓取不依赖开播状态：目录未定（重启后/尚未开播）时每轮检测前都补抓，
  单轮重试 3 次（每次间隔 3 秒）失败下一轮继续，直到开播首轮定下目录；
  输出目录本身仍要等确认开播后才创建
- 只有开播首轮仍无昵称才回退为纯频道标识目录，避免同一频道分裂出
...
- 开播首轮仍无昵称（各轮补抓均失败）时退化为 `./recordings/tiktok/<username>/`，
...
| 参数 | 作用 |
|------|------|
| `--detect-interval` | 未开播检测基础间隔，默认 210 秒 |
| `--detect-jitter` | 检测间隔随机抖动，默认 ±90 秒，即 120–300 秒 |
